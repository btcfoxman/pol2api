...
function renderAccounts() {
  ...
  $("#accountsBody").innerHTML = items.map((account) => {
    ...
    return `<tr><td><span class="cell-title"><span class="account-id mono">#${account.id}</span>${escapeHtml(account.name)}</span><span class="cell-sub">${escapeHtml(account.email || account.user_id || "")}</span></td>
      <td>${badge(!account.enabled && ["active", "pending"].includes(account.status) ? "disabled" : account.status || (account.enabled ? "pending" : "disabled"))}<span class="cell-sub" title="${escapeHtml(account.last_error)}">${escapeHtml(account.last_error)}</span></td>
      ...
      <td><span class="cell-title mono proxy" title="${escapeHtml(account.proxy_url)}">${escapeHtml(proxyLabel(account.proxy_url))}</span></td><td><span class="cell-title mono">${account.cdp_port || "未配置"}</span><span class="cell-sub">${account.password ? "密码登录" : account.auto_login ? "自动重连" : "手动"}</span></td><td>${fmtTime(account.last_checked_at)}</td>
      <td><div class="row-actions"><button class="icon-button" data-action="check" data-id="${account.id}" title="检测"><i data-lucide="activity"></i></button><button class="icon-button" data-action="${account.password ? "password-login" : "login"}" data-id="${account.id}" title="${account.password ? "重新密码登录" : "读取 CDP 会话"}"><i data-lucide="${account.password ? "key-round" : "monitor-up"}"></i></button><button class="icon-button" data-action="edit" data-id="${account.id}" title="编辑账号"><i data-lucide="pencil"></i></button><button class="icon-button" data-action="toggle" data-id="${account.id}" data-enabled="${account.enabled}" title="${account.enabled ? "禁用" : "启用"}"><i data-lucide="${account.enabled ? "pause" : "play"}"></i></button>${account.enabled ? "" : `<button class="icon-button danger" data-action="delete" data-id="${account.id}" title="删除"><i data-lucide="trash-2"></i></button>`}</div></td></tr>`;
  }).join("");
...
async function accountAction(button) {
  ...
  try {
    ...
    if (action === "password-login") await api(`/api/accounts/${id}/password-login`,{method:"POST"});
...
document.addEventListener("click", async (event) => {
  ...
  const accountButton = event.target.closest("[data-action='check'],[data-action='login'],[data-action='password-login'],[data-action='edit'],[data-action='toggle'],[data-action='delete']"); if (accountButton) await accountAction(accountButton);
...
$("#batchForm").addEventListener("submit",async event=>{event.preventDefault();const form=event.currentTarget;try{const result=await api("/api/accounts/batch-import",{method:"POST",body:JSON.stringify({text:form.elements.namedItem("text").value,start_login:form.elements.namedItem("start_login").checked,use_proxy_pool:form.elements.namedItem("use_proxy_pool").checked})});$("#batchResult").hidden=false;$("#batchResult").textContent=JSON.stringify({imported:result.imported,login_queued:result.login_queued,needs_session:result.needs_session-result.login_queued,errors:result.errors},null,2);toast(`已导入 ${result.imported} 个账号，${result.login_queued} 个正在登录`);if(result.needs_session){state.accountFilter="disabled";$$('#accountTabs button').forEach(item=>item.classList.toggle("active",item.dataset.filter==="disabled"));}await refresh();}catch(error){toast(error.message,true);}});
