...
Pollo.ai 协议网关。任务管理、账号池、设置界面和外部 API 形式沿用本地 ak2api，Pollo 上游适配重新实现。支持邮箱密码登录、已授权的 Cookie 会话和现有 Chrome CDP 会话；积分领取和自动注册暂不接入。
...
控制台支持 Cookie Header / Cookie JSON 导入、批量 JSON 或逐行 `邮箱|密码|代理` 导入、编辑、启用/禁用、并发设置、代理池分配、余额检测，以及从已打开的本机 CDP 端口读取会话。逐行导入默认后台密码登录；登录成功后检查余额，并仅启用额度达到阈值的账号。登录遇到平台验证时，账号保持禁用并显示“需验证”，可在浏览器完成验证后导入 Cookie。密码保存在服务端 SQLite，不在账号 API 中返回。读取 CDP 不会启动、导航或关闭用户浏览器，不使用 Playwright。容器内的本机 CDP 只适用于同容器浏览器；跨机器请导入 Cookie。
...
“视频生成方式”默认使用工具直接生成。打开“通过 Agent 生成视频”后，新任务沿用现有外部 API，通过 Pollo Agent 提交；服务会在原提示词末尾指定本次实际模型、时长、分辨率、比例和视频数量，并读取 Agent 的最终视频。已排队任务保持创建时选定的方式。Agent 若交付了与请求参数不符的视频，任务会报参数不匹配，而不会把错误规格的视频返回调用者。
当 `generate_audio=true`（默认值）时，追加语句还要求同步生成与画面匹配的声音，并将非静音音轨合成进成片；显式设置为 `false` 时则要求无声视频。
Agent 的工具参数也会在追加语句中逐项指定：`model`、`duration`、`resolution`、`aspect_ratio` 与 `options.generate_audio`。如果 Agent 工具不支持这些参数，应停止生成而不是改用自适应比例等替代规格。
Agent 模式可使用手动启用且余额充足的 `generation_restricted` 账号；直接生成模式仍排除这类账号，账号的限制记录不会被自动清除。
